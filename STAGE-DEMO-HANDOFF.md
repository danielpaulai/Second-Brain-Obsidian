...
> watching a person *borrow Danny's brain in real time*.
>
...
> "*This is my second brain. Every conversation, decision, framework, story — 10 years of work, queryable.
> What if your content creation, your decision-making, your team — could borrow your brain on demand,
> with your voice, your judgment, your context? Watch.*"
...
4. **Single-key triggers** — bind keys 1–9 to pre-baked demo scenarios (see §4). Pressing "1" should *immediately* start the cinematic flow.
...
**Problem:** between "user hits Enter" and "first answer token streams in" there's a 1–4s gap. Right now we show `ThinkingPulse`. On stage, this is the moment to *show the work*.
...
**Problem:** live queries can fail. On a stage with 500 people watching you don't want to debug a brain query. We need scripted scenarios that *feel* live but are bulletproof.
...
export const SCENARIOS = [
  {
    ...
    expectedNoteIds: ["frameworks/positioning", "icp_segments/founders-b2b", "stories/dana-pivot"],
...
- Has a custom intro narration card (e.g. "*Watch what happens when I ask my brain about positioning…*").
...
### P4 — Output that *looks* like a brain answering, not a chatbot
...
**Problem:** the demo's emotional payoff is "*your team can use this too.*" Today, `/ask` exists for team — but it's a stripped down version. Polish it for the second half of the talk.
...
| Question 2 (voice) | 1:10 | Hold spacebar, say: *"What's my best hook about pricing?"*, transcript appears live | spacebar |
...
- **The movie *Arrival*** — for the "the system is thinking and it's beautiful" energy. Specifically the heptapod ink-circles forming. Our `fireCinematic` already gestures at this.
...
3. **No fabrication.** The chat system prompts include hard "do not fabricate, cite the brain" rules. Skill files in `<vault>/_ai-danny/skills/*.md` enforce this. If a stage answer card has no source, it must say "*Generalizing — not in the brain*" and not invent.
...
| 3 | Build `ResearchOverlay.tsx` consuming `experimental_toolCallStreaming` from AI SDK. | Status text streams *during* tool calls, not after. |
...
- [ ] Holding spacebar records voice, releasing sends the question, transcript shows live in the HUD.
...
- [ ] Tool-call streaming makes the "researching" moment feel alive — no dead 3s pause.
...
Good luck. The hard part is already built — this sprint is about turning a working second brain into a stage moment people remember.
