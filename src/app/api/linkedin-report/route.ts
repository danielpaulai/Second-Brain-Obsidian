 ...
 * Deterministic chart data (computeLinkedInStats) + a narrative that answers the user's ACTUAL
 * request, written with a real REASONING pass: Anthropic Claude extended thinking by default
 * (`AI_MODEL=anthropic/...`), or OpenAI high reasoning effort if `AI_MODEL=openai/...`.
 */
type Picked = {
...
export async function POST(req: Request) {
  ...
  if (!query) query = "Analyse my LinkedIn — what's working, what's not, and what should I do next?";
  ...
  const stats = computeLinkedInStats(scope);
